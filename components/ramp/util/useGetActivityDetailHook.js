...
const useGetActivityDetailHook = () => {
  const [isLoading, setIsLoading] = useState(false);
  
  const fetchData = async (external_id) => {
    setIsLoading(true);

    const apiUrl = process.env.RAMP_API_URL + '/ramp/payment_transaction'; // Example API endpoint
...
export default useGetActivityDetailHook;