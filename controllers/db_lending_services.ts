...
export const getPoolActivitiesService = (poolId: string, network: string): Promise<PoolActivity[]> => {
    return new Promise(async (resolve, reject) => {
        try {
            connection.getConnection(function(err, conn) {
                if(err) {
                ...
                conn.query(
                    ...
                    (err, results, fields) => {
                        if(err) reject(err);
                        resolve(results as PoolActivity[]);
                });
                connection.releaseConnection(conn);
            })
        } catch (error) {
...
export const getUserPoolsService = (address: string): Promise<UserPool[]> => {
    return new Promise(async (resolve, reject) => {
        try {
            connection.getConnection(function(err, conn) {
                if(err) {
